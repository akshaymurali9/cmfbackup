...
export class MyAuthAuthenticationStrategyProvider implements Provider<AuthenticationStrategy | undefined> {
  ...
  async verifyToken(
    ...
    try {
      ...
      console.log("User retrieved from email in verify token" + user?.id + " " + user?.email);
  ...
  async verifyRoles(username: string | undefined) {
...
export class MyAuthActionProvider implements Provider<AuthenticateFn> {
  constructor(
    ...
    @inject.setter(AuthenticationBindings.CURRENT_USER) readonly setCurrentUser: Setter<UserProfile | undefined | RedirectRoute>,
  ...
  async action(request: Request): Promise<UserProfile | undefined | RedirectRoute> {
